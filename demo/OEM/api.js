...
(function(G) {
    ...
    api.scan = function(chip) {
        __es(api.scan, api.server + '/gap/nodes/?event=1&mac=' + api.hub + '&access_token=' + api.access_token,
            function(event) {0
    ...
    api.write = function(o) {
        ...
        $.ajax({
            type: 'get',
            url: api.server + '/gat/nodes/' + o.node + '/handle/' + o.handle + '/value/' + o.value + '/?mac=' + (o.hub || api.hub),
            headers: api.local ? '' : {
                'Authorization': api.authorization
            },
            dataType: 'text',
            success: function(data) {
                 // console.log('writeByhabdle --> success', data)
                o.success && o.success(data)
            },
            error: function(err){
                console.log('writeByhabdle --> err', err)
            }
        })
        return api
    }
